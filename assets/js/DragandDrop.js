console.log("Dentro de DragandDrop.js");

//Los evento de JS sona cciones o suscesos que ocurren en un docuemnto web y que pueden ser detectados y manejados por le código JS. Estos eventos puedes ser desencadenados por el usuario o generados automáticamente por le navegador
...
function start(e) {
    //funcion que se ejecuta con el evento ondragstart, recibe como parametro
    //el propio evento (como acción)
    console.log("start");
    e.dataTransfer.effectAllowed = "move"; //define el efecto para mover el elemento
    e.dataTransfer.setData("Data", e.target.id); //guarda en memoria (caché) la info del elemento
    //a mover ("Data":1)
    $("#" + e.target.id).css("opacity", "0.4");//mediante Jquery busca el elemento que en cuestión
    //y cambia sus propiedades CSS
    console.log(e.target.id)
    elemArrastrable = e.target.id //guardamos en la variable global el id del elemento en cuestión
}

function end(e) {//funcion que se ejecuta con el evento ondragend, recibe como parametro
    //el propio evento (como acción)
    console.log("end")
